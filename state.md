...
- **Task atual:** **0003 — Conformidade package-by-domain** (pré-E0b, branch `refactor/0003-conformidade-package-by-domain`, criada 2026-07-10): **implementada em 2026-07-10 (9 commits), aguardando auditoria pré-push.** Realinhamento completo ao ADR 0003 (`internal/catalogo/`, `internal/health/`, `cmd/morfeu`, `depguard` v2 ativo com fronteiras módulo↛módulo e domínio↛driver de infra, `sqlc.yaml`/Makefile/Dockerfile corrigidos) + teste HTTP real de `GET /filmes` (exigência da auditoria do fix). Suíte completa verde com `-race` via container; CA01–CA06 do PRD verificados (detalhe em `plan.md`). Não bloqueada pelas perguntas escaladas. Numeração: 0002 segue reservada à E0b (referências existentes).
- **PRD atual:** **0003** (`docs/prd/0003-conformidade-package-by-domain.md`, ativo, 2026-07-10) — conformidade package-by-domain. (PRD da 0002/E0b segue aguardando respostas do refinamento E0.)
